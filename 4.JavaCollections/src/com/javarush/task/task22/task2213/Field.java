...
public class Field {
    ...
    public void removeFullLines() {
        ...
        List<int[]> listOfLine = Arrays.asList(matrix);
        List<int[]> newListOfLine = new ArrayList<>();
        for (int[] line : listOfLine) {
            int full = 0;
            for (int i : line) {
                if (i == 1)
                    full++;
            ...
            if (full != width)
                newListOfLine.add(line);
            else {
                full = 0;
            }
        }
        while (listOfLine.size() != newListOfLine.size())
            newListOfLine.add(0, new int[width]);
        newListOfLine.toArray(matrix);
    }
}
